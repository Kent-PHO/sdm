...
2.  Export values from converter 'Risk reduction annual' to Excel, edit then re-import. This takes a bit longer but has some distinct advantages: - allows for bulk changes - keeps a record of changes made for reproducibility - can be done programmatically in theory using R.
...
8.  In Model Exports screen under 'Destination', select an existing Excel file
...
Note that risk factor trend values are static and applied consistently across all 25 years of the model run.
:::